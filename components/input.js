define(["utils", "base"], function (utils, baseClass) {
    ...
    var Component = function (config) {
        ...
        var inputStyle = {
            ...
            color: this.config.style.color || "#262626",
        ...
        var method_compositionstartname = config.comKey + "_compositionstart";
        var method_compositionstart = _this.pageview.plugin[method_compositionstartname];
        if (method_compositionstart) {
            this.input.bind("compositionstart", function (e) {
                !_this.disabled && method_compositionstart && method_compositionstart.call(_this.pageview.plugin, _this, {e: e});
            });
        }
        ...
        if (mode === "number") {
            this.$el.on('input', function (e) {
                ...
                if(isNaN(curValu)){
                    curValuTemp = curValu.replace(/[^0-9\.]/g, '');
                    if (curValu.indexOf('-') === 0){
                        curValuTemp = '-' + curValuTemp;
                    }
                    _this.setValue(curValuTemp);
                }
            ...
            this.$el.bind("keydown", function (e) {
                ...
                if(!(e.keyCode==189) && !(e.keyCode==190)&&!(e.keyCode==46)&&!(e.keyCode==8)&&!(e.keyCode==37)&&!(e.keyCode==39)&&!((e.keyCode>=48&&e.keyCode<=57)||(e.keyCode>=96&&e.keyCode<=105))){
                    e.stopPropagation();
                    e.preventDefault();
                    e.returnValue = false;
                    return;
        ...
        var change_name = config.comKey + "_change";
        var change_nameMethod = _this.pageview.plugin[change_name];
        if (change_nameMethod) {
            this.input.bind("change", function (e) {
                !_this.disabled && change_nameMethod && change_nameMethod.call(_this.pageview.plugin, _this, {e: e});
            });
        }
    }
