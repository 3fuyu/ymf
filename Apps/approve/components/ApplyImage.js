      ...
      this.components[this.imageKey] = {
        ...
        this.plugin[this.subComponentKey+"_click"] = function(sender,params){
            ...
            if(RepeatInstance.datasource.length>=max){
                _this.pageview.showTip({text:"最多上传"+max+"张图片",duration:1000,style:{width:"220px"}});
                return;
            }
            ...
            try{
                ...
                window.yyesn.client.selectAttachment(function(Re){
                    var data= Re.data;
                    if(data.length > last){
                        this.pageview.showTip({text:"最多上传5张图片",duration:1000,style:{width:"220px"}});
                    }
                    for(var i=0,j=last;i<j;i++){
                        RepeatInstance.addItem({src:data[i].path});
                    }

                },{type:1,maxselectnum:last});
            }catch(e){
            }
        }
