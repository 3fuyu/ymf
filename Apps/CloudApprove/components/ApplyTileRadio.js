...
define(["./ApplyBase", "utils", "../parts/commonLayout"], function(baseClass, utils, c) {
    function Component(config) {
        ...
        this.components[this.titleKey] = this.getTileCtlTitle(this.itemData.title );
        ...
        this.components[this.id] = {
            ...
            ref: true,
            items: [],
            root: ['item_select_icon', this.textKey],
            style: utils.processStyle({
                flexDirection: "row",
                flexWrap: "wrap",
        ...
        this.components['item_select_icon'] = {
            type: "radiobox",
            selectedClassName: "radio-group-selected",
            ref: true,
            radioStyle: {
                fontSize: 14,
                width: 18,
        ...
        this.plugin[this.id + "_itemclick"] = function(sender, params) {
            ...
            } else if(_this.itemData.hasOther){
    ...
    Component.prototype.bindInitValue = function(sender) {
        ...
        for (var i = 0, j = optionsRepeatArr; i < j; i++) {
    ...
    Component.prototype.getConfig = function(rowInstance) {
        ...
        if (value.length === 0) {
            ...
            Config.value = value[0].datasource.selectionId + '';
        }
        ...
        if (this.itemData.crux){
            ...
            selectedValue.forEach(function (item, index) {
                featureValue += item.name || item.datasource.name;
            });
