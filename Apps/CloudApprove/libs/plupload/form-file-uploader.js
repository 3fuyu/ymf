 ...
 *
 */
define(["../../parts/common", "utils", "./plupload.full.min"], function (c, utils, plupload) {
    ...
    formUploader.prototype = {
         ...
         */
        initUploader: function (pickerDiv, containerDiv, taskId, fileNumLimit, isShare) {//div.choose-file-btn
            //debugger;
            var _this = this;
                        ...
                        if (info.status === 200) {
                            ...
                            _this.parentPageview.ajax({
                                ...
                                success: function (data) {
                                    ...
                                    if (data.success) {
                                        _this.parentPageview.showTip({text: "上传成功", duration: 1000});
                                        var atta = data.data;
                                        _this.uploaderFileRefs[atta.id] = file;

                                        _this.parent.loadFilesData(data.data);

                                        _this.recordFileUploaded(uploaderId, atta);
                                    } else {
                                        _this.parentPageview.showTip({text: "上传失败：" + data.msg, duration: 2000});
